...
public class ZinoTicketVenteExtractor {
    ...
    private TicketVenteZino construireTicket(List<String[]> lignesTicket) {
        ...
        // Liste pour stocker tous les détails
        List<DetailZino> details = new ArrayList<>();
        ...
        for (String[] colonnes : lignesTicket) {
            ...
            switch (typeLigne.toUpperCase()) {
                ...
                case "DETAILS" -> {
                    ...
                    double quantite = parseDouble(colonnes[4]);
                    double prixUnitaire = parseDouble(colonnes[5]);
                    ...
                    // Créer un objet Detail pour chaque ligne
                    DetailZino detail = new DetailZino();
                    detail.setCodeProduit(codeProduit);
                    ...
                    if (data.getCodeProduitPrincipal() == null) {
                        data.setCodeProduitPrincipal(codeProduit);
                        data.setDesignationPrincipale(designation);
                    } else {
                        data.setCodeProduitPrincipal(data.getCodeProduitPrincipal() + "\n" + codeProduit);
                        data.setDesignationPrincipale(data.getDesignationPrincipale() + "\n" + designation);
                    }
                }
